<?php
	...
	class Mshopdaugia extends CI_Model
	{
	    /**
	     * summary
	     */
	    ...
	    public function sapDienRa($shop)
	    {
	    	...
	    	$this->db->where('sp.iNguoithem', $shop);
	    ...
	    public function getAuctua($start, $end, $limit, $shop) {
	    	$this->db->limit($limit);
	    	...
	    	return [
	    		'listPhien' => $phien,
	    		'tongPhien' => $this->getSoPhien($start, $end, $shop),
	    		'hinhAnh' 	=> $this->getHinhAnhSanPham($arrayMaSanPham),
	    	];
	    }
	    ...
	    public function getSoPhien($start, $end, $shop) {
	    	$session = $this->session->userdata('user');
	    ...
	    public function getCurrentErr()
	    {
	    	...
	    	$this->db->from('tbl_phiendaugia pdg');
	    	$this->db->join('tbl_ct_phiendaugia ctp', 'pdg.iMaphiendaugia = ctp.iMaphiendaugia', 'left');
	    	$this->db->where('dThoigianketthuc < SUBDATE(NOW(), INTERVAL 3 DAY)');
	    ...
	    {
	    	$this->db->where('iManguoidung', $shop);
